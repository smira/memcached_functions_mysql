...
my_bool memc_decrement_init(UDF_INIT *initid, UDF_ARGS *args, char *message);
long long memc_decrement(UDF_INIT *initid, UDF_ARGS *args, char *is_null, char *error);
void memc_decrement_deinit(UDF_INIT *initid);
...
my_bool memc_decrement_init(UDF_INIT *initid, UDF_ARGS *args, char *message)
{
  ...
  container= prepare_args(args, message, MEMC_DECREMENT, 1, 2);
  if (container == NULL)
    return 1;
...
long long memc_decrement(UDF_INIT *initid, UDF_ARGS *args,
                           ...
                           __attribute__ ((unused)) char *error)
{
  ...
  rc= memcached_decrement(&container->memc,
