...
long long memc_replace(UDF_INIT *initid, UDF_ARGS *args, char *is_null, char *error);
void memc_replace_deinit(UDF_INIT *initid);
my_bool memc_replace_by_key_init(UDF_INIT *initid, UDF_ARGS *args, char *message);
long long memc_replace_by_key(UDF_INIT *initid, UDF_ARGS *args, char *is_null, char *error);
...
my_bool memc_replace_by_key_init(UDF_INIT *initid, UDF_ARGS *args, char *message)
{
  ...
  container= prepare_args(args, message, MEMC_REPLACE_BY_KEY, 3, 4);
  if (container == NULL)
    return 1;
